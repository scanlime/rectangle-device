...
use rectangle_device_blocks::{
    dir::DirectoryBlock,
    package::Package,
    raw::{MultiRawBlockFile, RawBlockFile},
    BlockInfo, BlockUsage, Cid, PbLink,
};
use rectangle_device_player::{main_js, IndexTemplate, Template};
...
impl HLSPlayerDist {
    pub fn new() -> HLSPlayerDist {
        ...
        let links = vec![script.link(JS_FILENAME.to_string())];
    ...
    pub fn copy_blocks(&self) -> impl IntoIterator<Item = BlockInfo> {
        ...
        self.script
            .clone()
            .into_blocks()
            .map(move |block| usage.attach_to(block))
    }
}
...
impl HLSPlayer {
    ...
    pub fn from_link(
        ...
        let mut links = vec![html.link(HTML_FILENAME.to_string()), hls_link];
    ...
    pub fn into_blocks(self) -> impl IntoIterator<Item = BlockInfo> {
        ...
        self.html
            .into_blocks()
            .map(move |block| player_usage.attach_to(block))
            .chain(
                self.directory
                    .into_blocks()
